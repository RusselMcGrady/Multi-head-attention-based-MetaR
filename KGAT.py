...
class KGAT(nn.Module):
    ...
    def calc_kg_score(self, h, r, pos_t, neg_t):
        """
        h:      (kg_batch_size)
        r:      (kg_batch_size)
        pos_t:  (kg_batch_size)
        ...
        r_mul_h = torch.bmm(h_embed.unsqueeze(0), W_r).squeeze(0)                       # (kg_batch_size, relation_dim)
        r_mul_pos_t = torch.bmm(pos_t_embed.unsqueeze(0), W_r).squeeze(0)               # (kg_batch_size, relation_dim)
        r_mul_neg_t = torch.bmm(neg_t_embed.unsqueeze(0), W_r).squeeze(0)               # (kg_batch_size, relation_dim)
    ...
    def forward(self, *input, mode):
        ...
        if mode == 'predict_kg':
            return self.calc_kg_score(*input)

class KGATb(nn.Module):

    def __init__(self, args,
                 n_users, n_entities, n_relations,
                 user_pre_embed=None, item_pre_embed=None):

        super(KGATb, self).__init__()
        self.use_pretrain = args['use_pretrain']
    ...
    def calc_cf_loss(self, user_ids, item_pos_ids, item_neg_ids, A_in):
        if A_in is not None:
            self.A_in.data = A_in
    ...
    def calc_kg_loss(self, h, r, pos_t, neg_t, A_in):
        if A_in is not None:
            self.A_in.data = A_in
